...
def svd_learn_new(
    ...
    class PartialR:
        ...
        def reconstruct(self):
            ...
            S = np.real(
                np.diagonal(
                    self.R @ Zs_ @ np.transpose(Ys_, axes=(0, 2, 1)) @ self.R.T,
                    axis1=1,
                    axis2=2,
                ).T
            )
    ...
    def reconstruct_at(i, j):
        ...
        d, _, _, _ = np.linalg.lstsq(
            (R_.T @ Ys_[i] @ Ps_[i]).T, Os[i] @ np.ones(n), rcond=None
        )
        R = np.diag(d) @ R_.T
        return R
    ...
    def combine(parts):
        ...
        collectedRinvOrigin = [
            i for i, p in enumerate(parts) for _ in range(len(p.Rinv.T))
        ]

        dists = np.zeros(len(collectedRinv) * (len(collectedRinv) - 1) // 2)
        for k, ((p1, rinv1), (p2, rinv2)) in enumerate(
            itertools.combinations(zip(collectedRinvOrigin, collectedRinv), 2)
        ):
            dists[k] = np.linalg.norm(rinv1 - rinv2) ** 2 / (
                np.linalg.norm(rinv1) * np.linalg.norm(rinv2)
            )
            if p1 == p2:
                dists[k] = 10
        ...
        double_dists = [
            0 if i // 2 == j // 2 else dist_mtrx[i // 2, j // 2]
        ...
        def asgn_mtrx_bf(mass):
            ...
            if comp_rec(0) and verbose:
                print("timeout for asgnm")
        ...
        Ps = Ys @ Ps_
        S = np.real(
            np.diagonal(Zs @ np.transpose(Ys, axes=(0, 2, 1)), axis1=1, axis2=2).T
        )
        Ms = np.real(np.transpose(Ps / S, axes=(1, 2, 0)))
        return S, Ms
    ...
    if L is None:
        ...
        if mixture is not None:
            sigma_min = min(
                np.min(np.linalg.svd(X, compute_uv=False))
                for i in range(n)
                for X in [mixture.Ms[:, i, :], mixture.Ms[:, :, i]]
            )
    ...
    for _ in range(n_repeat):
        try:
            for g in range(max(groups)):
                ...
                if verbose:
                    print(f"=== group {g}: {states} i={i} j={j} {'='*50}")
            ...
            if em_refine_max_iter > 0:
                learned_mixture = em_refine(
                    learned_mixture, em_refine_max_iter=em_refine_max_iter
                )
            learned_mixture.normalize()

            d = (
                Distribution.from_mixture(learned_mixture, 3).dist(sample)
                if n_repeat > 1
                else 0
            )
            if d < best_dist:
...
def em_learn(
    ...
    for n_iter in range(max_iter):
        ...
        if ll_stop is not None and np.max(np.abs(prev_lls - lls)) < ll_stop:
            break
        prev_lls = lls
        ...
        flat_mixture = cond_probs.transpose() @ (
            flat_trails * trail_probs[:, np.newaxis]
        )
        # normalize:
        ...
        if verbose is not None:
            ...
            learned_distribution = Distribution.from_mixture(
                learned_mixture, sample.t_len
            )
            print(
                "Iteration {}: recovery_error={} tv_dist={}".format(
                    n_iter + 1,
                    (
                        verbose.recovery_error(learned_mixture)
                        if isinstance(verbose, Mixture)
                        else np.inf
                    ),
